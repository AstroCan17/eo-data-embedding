...
def fetch_bundle(dest: str = "demo", url: str = DEFAULT_BUNDLE_URL) -> Path:
    ...
    with urllib.request.urlopen(url) as r:  # noqa: S310  # nosec B310 (trusted release URL / user-overridable)
        data = r.read()
...
def serve(bundle: str = "demo", data_root: str = "data/", port: int = 7860):
    ...
    def run(pos, k):
        ...
        verdict = f"**Probe prediction:** {classes[pred]} ({proba:.0%} conf) — true class: {classes[true]} · {mark}"
    ...
    demo.launch(server_name="0.0.0.0", server_port=port)  # nosec B104 (demo UI: bind all interfaces)
