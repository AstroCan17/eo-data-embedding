...
def _eurosat(n: int, size: int = 224):
    ...
    for i in idx:
        ...
        x = torch.nn.functional.interpolate(x.unsqueeze(0), size=(size, size),
                                            mode="bilinear", align_corners=False)[0]
        imgs.append(x); labels.append(int(s["label"]))
...
def main() -> int:
    ...
    from geo_embed_eo import store, search, probe

    # --- data ---
    imgs, labels = (_eurosat(args.n) if args.eurosat else _synthetic(args.n))
    print(f"[smoke] dataset: {'EuroSAT' if args.eurosat else 'synthetic'} "
    ...
    with tempfile.TemporaryDirectory() as td:
        p = store.save_embeddings(Path(td) / "emb.parquet",
                                  ids=range(len(labels)),
                                  vectors=X, modality=["s2"] * len(labels), labels=labels)
    ...
    for shots in (5, 20, 50):
        ...
        print(f"          shots={shots}: macro_f1={r['macro_f1']:.3f} "
