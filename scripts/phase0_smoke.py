...
def _eurosat(n: int, size: int = 224):
    ...
    for i in idx:
        ...
        x = torch.nn.functional.interpolate(x.unsqueeze(0), size=(size, size), mode="bilinear", align_corners=False)[0]
        imgs.append(x)
...
def main() -> int:
    ...
    log.info(f"dataset: {'EuroSAT' if args.eurosat else 'synthetic'} n={len(labels)} classes={len(set(labels))}")
