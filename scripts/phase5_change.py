...
the ground-truth change mask (per-tile). Zero training — pure embedding geometry.
...
def main() -> int:
    ...
    ap.add_argument(
        "--root",
        default=cfg_get(cfg, "data.root", "data/"),
        help="OSCD dir (can be an external/NAS mount, e.g. /datasets/oscd)",
    )
    ...
    ap.add_argument(
        "--frac",
        type=float,
        default=cfg_get(cfg, "change.frac", 0.05),
        help="changed-pixel fraction for a tile to count as changed",
    )
    ...
    log.info(f"loading OSCD ({args.split}) ...")
    ...
    for p in pairs:
        ...
        log.info(f"pair {p['id']}: {len(t1)} tiles, {int(gts[-1].sum())} changed")

    s = np.concatenate(scores)
    g = np.concatenate(gts)
    log.info(f"{len(s)} tiles total, {int(g.sum())} changed ({100 * g.mean():.1f}%)")
